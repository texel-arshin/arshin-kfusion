...
namespace knn {
    ...
    static const int CUDA_STACK = 100; // fixed size stack elements for each thread, increase as required. Used in SearchAtNodeRange.
    ...
    __device__ bool CheckIgnored(int idx, int *ignored_indices, int ignored_count)
    {
    ...
    __device__ void
    SearchAtNode(const CUDA_KDNode *nodes, const int *indexes, const Point *pts, int cur, const Point &query, int *ret_index, float *ret_dist,
                 int *ret_node, int *ignored_indices, int ignored_count) {
    ...
    __device__ void
    SearchAtNodeRange(const CUDA_KDNode *nodes, const int *indexes, const Point *pts, const Point &query, int cur, float range, int *ret_index,
                      float *ret_dist, int *ignored_indices, int ignored_count) {
    ...
    __device__ void SearchOne(const CUDA_KDNode *nodes, const int *indexes, const Point *pts, const Point &query, int *ret_index, float *ret_dist, int *ignored_indices, int ignored_count) {
    ...
    __device__ void CudaKDSearch(const CUDA_KDNode *nodes, const int *indexes, const Point *pts, const Point &query, int *ret_index, float *ret_dist, int k_neighbours)
    {
    ...
    __global__ void
    SearchBatch(const CUDA_KDNode *nodes, const int *indexes, const Point *pts, int num_pts, Point *queries, int num_queries, int *ret_index,
                float *ret_dist) {
        ...
        CudaKDSearch(nodes, indexes, pts, queries[idx], &ret_index[idx], &ret_dist[idx]);
    }
    ...
    void CUDA_KDTree::Search(const vector <Point> &queries, vector<int> &indexes, vector<float> &dists) {
        ...
        SearchBatch << < blocks, threads >> >
                                 (m_gpu_nodes, m_gpu_indexes, m_gpu_points, m_num_points, gpu_queries, queries.size(), gpu_ret_indexes, gpu_ret_dist);
