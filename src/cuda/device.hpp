...
__kf_device__ half2 kfusion::device::pack_tsdf (float tsdf, float weight)
{ return make_half2 (__float2half_rn (tsdf), __float2half_rn(weight)); }

__kf_device__ float kfusion::device::unpack_tsdf(half2 value, float& weight)
{
    weight = __half2float(value.y);
    return __half2float(value.x);
}
__kf_device__ float kfusion::device::unpack_tsdf (half2 value) { return __half2float (value.x); }
...
namespace kfusion
{
    namespace device
    {
        constexpr float MIN_TSDF_WEIGHT = 0.01;
        ...
        template<> __kf_device__ half2 gmem::LdCs(half2* ptr);
        template<> __kf_device__ void gmem::StCs(const half2& val, half2* ptr);
    }
}
...
//
//#else
    template<> __kf_device__ half2 kfusion::device::gmem::LdCs(half2* ptr) { return *ptr; }
    template<> __kf_device__ void kfusion::device::gmem::StCs(const half2& val, half2* ptr) { *ptr = val; }
//#endif
