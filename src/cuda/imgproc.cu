...
//                half tmp = __float2half_rn(depth(y, x) * lambda * 0.001f);
//                if (depth(y,x)>0)
//                {
//                    //half a = dists(y,x);
//                    float g = __half2float(dists(y,x));
//                    float d = depth(y,x);
//                    d = __half2float(__float2half_rn(d/1000.));
//                    printf("(%d, %d) DISTS: %f, lambda %f, tmp %f\n", x,y, g, lambda, __half2float(tmp));
//                    printf("(%d, %d) DEPTH: %f\n", x,y, d);
//                }
            }
        }
