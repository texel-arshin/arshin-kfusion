...
namespace knn {
    ...
    bool PointSorter::operator()(const int a, const int b) {
        vector <Point> &pts = *tree_ptr->m_pts;

        return pts[a].coords[tree_ptr->m_current_axis] < pts[b].coords[tree_ptr->m_current_axis];
    }
    ...
    void KDtree::Split(KDNode *cur, KDNode *left, KDNode *right) {
        ...
        sort(cur->indexes.begin(), cur->indexes.end(), PointSorter(this));
