...
namespace kfusion
{
    namespace cuda
    {
        ...
        {
        public:
            TsdfVolume(const Eigen::Vector3i& dims);
            virtual ~TsdfVolume();
            ...
            virtual void integrate_nowarp(const Dists& dists, const Affine3f& camera_pose, const Intr& intr);
            virtual void integrate_cpu(const Dists& dists, const Affine3f& camera_pose, const Intr& intr, cpu_warp::WarpField_Host *wf_host_ptr);
            ...
            virtual void query_volume_with_grad(const DeviceArray<float> &verts, DeviceArray<float> &tsdfs, DeviceArray<float> &tsdfs_grad);
